...
function playGame(){
	...
	function usersTurn(){
		...
		colors.onclick = function(e){
			// get a reference to what the user clicked
			let userColor = e.target.id;
			
			if (isMatch(userColor, simonColorArray[target])){ // if it matches simons, at the target index...
				target ++; // increment the target

				if (target === round){ // if the target is equal to the round...
					setTimeout(nextRound, 2000); // go to the next round
				} else {
					usersTurn(); // otherwise keep checking on each user click
				}
